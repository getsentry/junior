...
import type { AutomationRun } from "@sentry/junior/api/schema";

import { useAutomationRunsData } from "../../api";
...
function emptyRunsText(input: {
  ...
  return "No visible automations have run yet.";
}

function AutomationRunRow(props: { run: AutomationRun }) {
  const { run } = props;
  const navigate = useNavigate();
  const title =
    run.title?.trim() ||
    (run.conversationId ? run.automationTitle : "No conversation");
  const costLabel =
    formatCostSummary(
  ...
  const tokensLabel =
    run.totalTokens === undefined ? "—" : formatCompactNumber(run.totalTokens);
...
function runStatusTone(status: AutomationRun["status"]): StatusChipTone {
