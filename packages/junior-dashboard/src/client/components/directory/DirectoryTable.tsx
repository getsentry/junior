  ...
  return (
    <div aria-label={props.ariaLabel} className="min-w-0">
      <div
        aria-hidden="true"
        className={cn(
          "grid items-center gap-4 border-b border-dashboard-border-subtle bg-dashboard-overlay-soft px-4 py-2.5 font-mono text-xs uppercase tracking-[0.1em] text-dashboard-text-muted max-md:hidden",
          directoryGridClass,
        )}
      >
...
export function DirectoryRow(props: { children: ReactNode; to: string }) {
  return (
    <Link
      className={cn(
        "group grid min-w-0 items-center gap-4 border-b border-dashboard-border-subtle px-4 py-3.5 text-inherit no-underline transition-colors last:border-b-0 hover:bg-dashboard-fill-soft max-md:grid-cols-3 max-md:gap-x-3 max-md:gap-y-4",
        directoryGridClass,
      )}
      to={props.to}
    >
