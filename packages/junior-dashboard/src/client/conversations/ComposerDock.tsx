  ...
  return (
    <div
      className={cn(
        ...
        "flex w-full min-h-0 max-h-[min(calc(var(--dashboard-viewport-height,100dvh)*0.55),24rem)] shrink-0 flex-col overflow-hidden bg-dashboard-bg px-2 pt-1.5 pb-[var(--dashboard-composer-dock-padding,0.375rem)] md:max-h-none md:overflow-visible md:px-7 md:pt-3 md:pb-5",
        props.className,
      )}
      data-composer-dock=""
    >
      {props.above ? (
        <div className="mx-auto w-full max-w-[52.5rem] min-h-0 min-w-0 shrink overflow-y-auto overscroll-contain md:overflow-visible">
          {props.above}
        </div>
      ) : null}
      <div className="mx-auto w-full max-w-[52.5rem] min-w-0 shrink-0">
        {props.children}
      </div>
    </div>
  );
