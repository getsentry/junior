...
function toolFixture(
  response: Response = Response.json({
    result: "Failed",
    jobs: [
      { name: "create-sentry-release", result: "Failed", state: "Completed" },
...
describe("GoCD stage", () => {
  it("returns the stage result, jobs, failed job names, and a stable link", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith({
      ...
      request: expect.objectContaining({
        method: "GET",
        url: "https://gocd.example.com/go/api/stages/deploy-getsentry-backend-de/7/deploy-primary/1",
      }),
    });
