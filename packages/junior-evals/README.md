...
- `memory_extraction`: run passive Memory extraction after completed turns. It is off by default because it adds model calls and a background task to every turn. The Memory evals turn it on through `memoryEvals`.
