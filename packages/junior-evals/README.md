...
- After all e2e shards finish, `evals / report` combines results, publishes the suite summary, and posts an `eval score` Check Run whose title shows the pass rate and required floor.
- The current e2e floor is `EVAL_MIN_PASS_RATE=0.8` (`80%` of cases passed). `vitest-evals` owns the aggregate gate: its Check Run and report step fail below the floor, while individual case misses are warnings when the gate passes. Missing shard result files or setup/runtime crashes before results are written remain hard failures.
