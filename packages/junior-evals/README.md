...
- Integration cases run with deterministic action review (`EVAL_ACTION_REVIEW=allow` in the integration config). Every reviewable tool action is allowed, so a live Guardian "ask" cannot fail a wiring case. Behavioral cases keep the live Guardian reviewer, and `evals/guardian/**` owns Guardian decisions.
