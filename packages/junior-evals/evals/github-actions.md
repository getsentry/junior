...
- `Output-router evals` runs visible-reply prepare conversation cases when those eval files/harness changed, `output-router.ts` changed, or the PR has `trigger-evals-output-router` / `trigger-evals`
...
Guardian evals only need gateway credentials. Behavioral, integration, and output-router evals still need gateway plus sandbox access.
...
7. For output-router runs, confirm the `output-router / run` job summary published and the job completed. Delivered-reply or silence mismatches fail that job hard.
