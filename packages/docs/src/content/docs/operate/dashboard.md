...
For a private conversation, the transcript is available only when the signed-in verified email matches the owning root conversation actor's persisted verified email. Every REST conversation object reports that match through `isParticipant`; other viewers receive redacted metadata. Top-level conversation summaries and details include usage from every descendant, even when those children are not in the current event projection; a directly fetched child reports only its own usage.
