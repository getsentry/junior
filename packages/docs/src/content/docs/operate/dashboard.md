...
For a private conversation, the transcript is available only when the signed-in verified email matches a persisted verified participant email. Every REST conversation object reports that match through `isParticipant`; other viewers receive redacted metadata.
