...
| `AI_MODEL`                                  | No          | Deprecated for profile configuration. Defines the legacy `standard` profile and remains the fallback for `AI_FAST_MODEL`. Use `createApp({ defaultProfile, profiles })`. Defaults to `xai/grok-4.5`.                              |
...
| `AI_HANDOFF_MODEL`                          | No          | Deprecated. Defines the legacy `handoff` profile. Use `createApp({ defaultProfile, profiles })`. Defaults to `openai/gpt-5.6-sol`.                                                                                               |
| `AI_MODEL_PROFILES`                         | No          | Deprecated. JSON object mapping profile names to model IDs. Use `createApp({ defaultProfile, profiles })`. Names must match `^[a-z][a-z0-9_-]*$`. Overrides `AI_MODEL` / `AI_HANDOFF_MODEL` for matching keys.                      |
