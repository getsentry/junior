...
- Node.js 24
- pnpm
- A Postgres database with the `vector` (pgvector) and `btree_gin` extensions for Junior SQL records and [Memory](/concepts/memory/)
...
`junior init` creates the app entrypoint, Nitro config, Vercel config, TypeScript config, CI workflow, app context files, local plugin and skill directories, `.env.example`, and a `plugins.ts` with maintenance enabled by default. Memory is built into `@sentry/junior`.
...
Junior requires a Postgres database that can enable the `vector` and
`btree_gin` extensions before you run migrations. Local Postgres URLs automatically
...
New apps created with `junior init` already have a `plugins.ts` file with maintenance enabled. To add more packaged plugins, install the packages and add them to the existing plugin set.
...
Scheduled automation tools and heartbeat behavior are built into Junior, while
`githubPlugin()` enforces Git commit attribution. See [Automations](/concepts/automations/)
