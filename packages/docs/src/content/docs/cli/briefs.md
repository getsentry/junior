...
Run the default prompt and model against a snapshot:
...
Use `--model <id>` or `--prompt <file>` to compare generation settings. Add `--turn-by-turn` to generate one version for each completed turn in the snapshot.
