...
| `afterMcpTool(ctx)`      | Run junior-owned side effects after a successful hosted MCP tool call on the lazy `searchMcpTools` / `callMcpTool` path. Prefer this for conversation annotations instead of inventing a parallel tool contract.                                                                  |
