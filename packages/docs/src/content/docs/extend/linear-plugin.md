...
Junior creates and updates issues through native `linear_createIssue` and `linear_updateIssue` tools backed by the hosted MCP connection. The create tool links created issues to the Junior conversation and prevents an uncertain retry from creating a duplicate. Reads, comments, and other workflow operations continue to use Linear's hosted MCP tools.
