...
- If the runtime names a missing or rejected service connection, stop and report the operator repair it requests. Service mode cannot be repaired with user OAuth. Otherwise, if a Sentry API call returns `401`, or clearly says the token is invalid, expired, revoked, or unauthorized, rerun the real Sentry command once and let the runtime trigger a reconnect flow when available.
