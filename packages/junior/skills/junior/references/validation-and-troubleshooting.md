...
| Plugin does not load in app                                                               | Package installed but it is missing from `defineJuniorPlugins(...)`, or local files are outside `app/plugins`. | Add the package name or trusted factory to the shared plugin set, or move files under `app/plugins`. |
