...
| `oauth requires credentials`                                                              | OAuth block has no credential delivery config.                                                                 | Add `credentials.type: oauth-bearer`, or `plugin-managed` when trusted hooks issue credentials.            |
| `oauth requires credentials.type "oauth-bearer" or "plugin-managed"`                      | OAuth was paired with unsupported credentials.                                                                 | Use bearer OAuth credentials, plugin-managed credentials, or remove `oauth`.                               |
| `uses plugin-managed credentials and its trusted registration must include`               | Plugin-managed manifest loaded without both trusted egress grant and credential issuance hooks.                | Register matching `grantForEgress` and `issueCredential` hooks through the runtime-safe plugin module.     |
