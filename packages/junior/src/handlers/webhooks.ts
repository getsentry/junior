...
export async function handlePlatformWebhook(
  ...
  return withContext(requestContext, async () => {
    ...
    let runWithPlatformContext = <T>(fn: () => T): T => fn();
    if (platform === "slack") {
      const { prepareSlackWebhookRequest } =
        await import("@/chat/slack/webhook-preprocess");
      const prepared = await prepareSlackWebhookRequest({
        bot,
        request,
        waitUntil,
      });
      rebuiltRequest = prepared.request;
      runWithPlatformContext = prepared.runWithWorkspace;
    }
    ...
    try {
      return await withSpan(
        ...
        async () => {
          try {
            const response = await runWithPlatformContext(() =>
