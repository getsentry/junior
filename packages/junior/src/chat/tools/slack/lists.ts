...
export function inferListColumnMap(
  schema: SlackListsSchemaColumnResponse[] = [],
): ListColumnMap {
  const pick = (
    predicate: (column: SlackListsSchemaColumnResponse) => boolean,
  ): string | undefined => schema.find(predicate)?.id;

  return {
    titleColumnId: pick((column) => {
      const key = normalizeKey(column.key);
      return (
        column.is_primary_column ||
        key.includes("title") ||
        key.includes("task") ||
        key.includes("name")
      );
