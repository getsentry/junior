...
export async function visitVisibleTranscripts(args: {
  ...
  conversationStore: Pick<TranscriptConversationStore, "listByActivity">;
  resultLimit: number;
  visit: (access: TranscriptAccess) => Promise<boolean | void>;
}) {
  const pageLimit = Math.min(
    MAX_SCAN_LIMIT,
    Math.max(100, args.resultLimit * 10),
  );
  let offset = 0;
  let scannedCount = 0;
...
async function slackSourceLink(args: {
  access: TranscriptAccess;
  deps: TranscriptToolResolvedDeps;
  includeLinks: boolean;
  messageTs: string | undefined;
}): Promise<string | undefined> {
  if (!args.includeLinks || !args.access.slackChannelId || !args.messageTs) {
    return undefined;
  }
...
export async function linkForMessage(args: {
  ...
  return await slackSourceLink({
    access: args.access,
    deps: args.deps,
    includeLinks: args.includeLinks,
    messageTs:
      args.message.meta?.slackTs ??
      slackThreadTsFromConversationId(args.access.conversation.conversationId),
  });
}
