...
/** Return a human-readable status label for a completed tool result (no input context). */
export function formatToolResultStatus(toolName: string): string {
  const known: Record<string, string> = {
...
/** Return a human-readable status label for a completed tool result, enriched with input details. */
export function formatToolResultStatusWithInput(
  toolName: string,
  input: unknown,
): string {
  const obj =
