...
/** Fetch basic channel metadata via conversations.info. */
export async function getChannelInfo(
  ...
  if (!normalized) {
    throw new Error("Channel info lookup requires a valid channel ID");
  }

  const response = await withSlackRetries(
    () => client.conversations.info({ channel: normalized }),
    3,
