...
function buildClassifierSystemPrompt(profileNames: string[]): string {
  return [
    ...
    "Use medium for normal assistant work: substantive explanations, source-backed checks, thread follow-ups, tool choice, likely tool use, bounded or localized code changes with clear instructions, ambiguous asks, multi-step analysis, or anything where a confident but shallow answer would be risky.",
    "Use high for difficult or tool-heavy work: debugging/root-cause analysis, architecture decisions, broad or multi-file implementation, research-heavy work, non-trivial drafting, or explicit requests to be thorough.",
    "Use xhigh only when the user explicitly requests maximum-depth reasoning or the task is unusually broad, ambiguous, and risky across multiple systems. Do not choose xhigh solely because a task involves code, debugging, architecture, research, or multiple files.",
    "Code writing, editing, review, and debugging must use profile handoff and never use low. Bounded or localized code work with clear instructions uses medium unless a higher-tier criterion applies.",
