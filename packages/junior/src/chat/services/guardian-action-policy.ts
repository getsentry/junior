...
- Reversible creates and bounded updates in systems the organization already uses for the requested work are ordinary medium-risk work when the current actor's intent covers the destination and material fields. This includes opening a pull request, creating a tracking issue or ticket, posting a requested update in the active conversation, or creating a one-time reminder. Judge the actual side effects, not the product name of the tool.
