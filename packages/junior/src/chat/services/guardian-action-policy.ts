...
- Creating a tracking issue or ticket in Linear or GitHub is ordinary medium-risk work when the current actor explicitly asked to file or create it, or affirmed a concrete create proposal that already names the destination and material fields. Do not ask again merely because the tool is open-world, multi-field, uses connected credentials, or carries a destructive annotation on a create-shaped write. Still ask when the destination team or repository is missing or ambiguous, the write updates or rewrites an existing issue without clear authorization, or the proposed fields go beyond the requested scope.
