...
const TOOL_POLICY_RULES = [
  ...
  "- Event tasks list for the current Slack channel or DM, not one thread. Update and delete also work for public tasks from another channel in the same Slack workspace when the task id is known. When listing them, use createdBy to explain creator-only credential changes and warn when triggerAvailable is false; an unavailable task remains stored but cannot receive events until its plugin event is enabled again.",
