...
function buildOutputSection(): string {
  ...
  return [
    ...
    "- Use Slack-flavored Markdown: **bold** section labels, `code`, [text](url) links, bullet lists, and fenced code blocks. No tables. For links always use `[text](url)`; never use angle-bracket autolink format (`<url>`). When the answer primarily lists several URLs, show each URL bare on its own line rather than as a labeled link.",
