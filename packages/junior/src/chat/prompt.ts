...
const TOOL_POLICY_RULES = [
  ...
  "- When a tool result includes a subscription, the runtime already watches those events; do not call watchResourceEvents for them. When a tool result includes a subscribable resource with suggestedEvents, use watchResourceEvents only for those remaining high-signal provider changes that serve the user's current intent; do not create scheduled polling tasks for events the watch can deliver. If suggestedEvents is empty or omitted, do not invent a watch. Write a concise intent summary, and tell the user when the temporary watch expires. Stop only the requested watch by id unless the user explicitly asks to stop every watch in the thread.",
