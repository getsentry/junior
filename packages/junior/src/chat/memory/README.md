...
Memory is a Junior core feature. It stores durable facts, recalls relevant
facts into prompts, and learns candidates from completed sessions. SQL
schemas, exported types, tools, and tests are authoritative.

`feature.ts` registers Memory through the core feature contract in
`../plugins/core-features.ts`. `createApp()` always installs it; the
`@sentry/junior/memory` entry exposes the supported API to apps.

## Surfaces

- `createMemory`, `archiveMemory`, `listMemories`, and `searchMemories` are
  model-visible tools registered by `feature.ts` under the `memory` namespace.
  `archiveMemory` can forget public memory or private memory owned by the
  current User.
...
- The Drizzle schema in `../../db/schema/memory.ts` and the core migrations
...
- `AI_MEMORY_MODEL` or `createApp({ memory: { modelId } })` selects the
...
- `createApp({ memory: { disableExtraction: true } })` disables passive session
  extraction. The two settings are independent and do not disable explicit
  memory tools.
- `createApp({ memory: { enabled: false } })` is an emergency switch. It keeps
  stored memory events rendering and removes every other Memory surface.
...
Follow `../../../../policies/data-redaction.md`,
`../../../../policies/security.md`, and the registration contract in
