...
export async function executeCredentialedEgressRequest(input: {
  ...
  // Host plugin operations need body-aware grants too. Keep sandbox body
  // inspection on its existing path; do not buffer arbitrary CLI uploads here.
  const bodyForGrantSelection =
    operation ||
    githubBodyInspection({
      provider,
