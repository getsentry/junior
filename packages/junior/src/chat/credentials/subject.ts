...
/** Create a core-signed user credential subject for a verified Slack DM turn. */
export function createSlackDirectCredentialSubject(input: {
  ...
  if (!channelId || !teamId || !userId || !secret || !isDmChannel(channelId)) {
    return undefined;
  }

  const allowedWhen = "private-direct-conversation";
  return {
    type: "user",
    userId,
    allowedWhen,
    binding: {
      ...
      signature: signPayload(
        secret,
        buildPayload({ allowedWhen, teamId, channelId, userId }),
      ),
    },
  };
}
...
export function verifySlackDirectCredentialSubject(input: {
  ...
  const { subject } = input;
  const binding =
    subject.binding && typeof subject.binding === "object"
      ? subject.binding
      : undefined;
  if (
    subject.type !== "user" ||
