...
/** Create a workspace facade that boots the sandbox only when a tool needs it. */
export function createLazySandboxWorkspace(args: {
  executor: Pick<SandboxExecutor, "createSandbox" | "getSandboxId">;
  ...
  const getSandbox = (reason: SandboxBootReason): Promise<SandboxWorkspace> => {
