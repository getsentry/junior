...
import { listScheduledAutomationsForTeam } from "../tasks";
...
function taskMatchesQuery(task: ScheduledAutomation, query: string): boolean {
...
export function createSlackScheduleListAutomationsTool(
  ...
  return zodTool({
    ...
    inputSchema: z
      .object({
        ...
        query: z
          ...
          .max(200)
          .nullable()
          .describe("Text to match in the requester's scheduled automations.")
          .optional(),
      })
      .strict(),
    ...
    execute: async (input) => {
      ...
      const matching = (
        await listScheduledAutomationsForTeam(getDb(), destination.teamId)
      )
        .filter((task) => {
