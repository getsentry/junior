...
const { assertPublicUrlMock, fetchTextWithRedirectsMock, withTimeoutMock, extractWebFetchResponseMock } = vi.hoisted(() => ({
...
describe("web fetch tool", () => {
  ...
  it("uses a single fetch path for non-image responses", async () => {
    ...
    const result = await execute({ url: "https://example.com/article", max_chars: 1200 }, {} as never);
    ...
    expect(extractWebFetchResponseMock).toHaveBeenCalledWith(safeUrl, expect.any(Response), 1200);
  });
});
