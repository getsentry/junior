...
import { rankMemoryMatches, type MemoryMatch } from "@/chat/memory/ranking";
import type { MemoryRecord } from "@/chat/memory/store";
...
describe("memory retrieval ranking", () => {
  it("promotes a memory supported by both retrieval methods", () => {
    const ranked = rankMemoryMatches(
      [
        match("vector-only", { vector: { rank: 1 } }),
        match("supported", { vector: { rank: 2 } }),
