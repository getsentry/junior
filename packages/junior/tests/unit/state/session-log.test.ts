...
describe("agent session log store", () => {
  ...
  it("commits Pi messages and plugin session state together", async () => {
    vi.useFakeTimers();
    ...
    await expect(
      ...
      { createdAtMs: 1_000, value: { memoryIds: ["mem_1"] } },
    ]);
    await expect(
      loadPluginSessionState({
        store,
  ...
  it("filters plugin session state to the active projection", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(1_000);
    const store = memoryStore();
    const oldMessage: PiMessage = {
      role: "user",
      ...
      pluginSessionState: [
        {
          plugin: "memory",
          appends: [
            { key: "injected_memories", value: { memoryIds: ["old"] } },
          ],
        },
      ],
      ttlMs: 60_000,
      ...
      pluginSessionState: [
        {
          plugin: "memory",
          appends: [
            { key: "injected_memories", value: { memoryIds: ["new"] } },
          ],
        },
      ],
      ttlMs: 60_000,
    ...
    ).resolves.toEqual([{ createdAtMs: 2_000, value: { memoryIds: ["new"] } }]);
    await expect(
      loadPluginSessionState({
        store,
        conversationId: "conversation-1",
    ...
    ).resolves.toEqual([{ createdAtMs: 1_000, value: { memoryIds: ["old"] } }]);
  });
