...
import { listMemories } from "@/chat/memory/viewer";
...
function recallModel(
  ...
  return {
    // @ts-expect-error test model returns one fixed object shape
...
`,
  ...
  it("admits more than five relevant memories when they fit the prompt budget", async () => {
    ...
    try {
      ...
      const admittedIds = (
        contribution.context.content.memories as Array<{ id: string }>
