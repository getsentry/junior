...
`,
  ...
  it("runs journaled TypeScript state migrations exactly once", async () => {
    const stateAdapter = getStateAdapter();
    await stateAdapter.connect();
    ...
    await stateAdapter.appendToList(
      `junior:agent_turn_session:conversation:${CONVERSATION_ID}:index`,
      summary,
    );
    await stateAdapter.set(
  ...
  it("migrates legacy conversation work before SQL conversation backfill", async () => {
    ...
    try {
      ...
      const results = [
        await migrateRedisConversationState(context),
