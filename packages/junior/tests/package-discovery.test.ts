...
async function writePluginPackage(nodeModulesRoot: string, packageName: string): Promise<string> {
...
describe("plugin package discovery", () => {
  it("discovers plugin content from node_modules even when package.json has no dependencies", async () => {
    ...
    const packageRoot = await writePluginPackage(nodeModulesRoot, "@acme/junior-plugin-demo");
    ...
    expect(discovered.tracingIncludes).toContain("./node_modules/@acme/junior-plugin-demo/plugin.yaml");
    expect(discovered.tracingIncludes).toContain("./node_modules/@acme/junior-plugin-demo/skills/**/*");
  });
  ...
  it("keeps nearest node_modules package when duplicate package names exist", async () => {
    ...
    const nearPackageRoot = await writePluginPackage(nearNodeModulesRoot, "@acme/junior-plugin-demo");
    ...
    expect(discovered.manifestRoots.some((candidate) => candidate.startsWith(farNodeModulesRoot))).toBe(false);
  });

  it("resolves explicit packageNames through node_modules symlinked packages", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "junior-package-discovery-"));
    const linkedPackageSource = path.join(tempRoot, "packages", "junior-plugin-link");
    const linkedPackageInNodeModules = path.join(tempRoot, "node_modules", "@acme", "junior-plugin-link");

    await fs.mkdir(path.join(linkedPackageSource, "skills", "demo"), { recursive: true });
    await fs.writeFile(path.join(linkedPackageSource, "plugin.yaml"), "name: demo\ndescription: demo\n", "utf8");
    await fs.mkdir(path.dirname(linkedPackageInNodeModules), { recursive: true });
