...
describe("Conversation mailbox Turn work", () => {
  ...
  it("collects a burst of events into one Turn behind a renewable debounce", async () => {
    ...
    try {
      ...
      const firstMessage = createEventInboundMessage({
        event: {
          eventKey: "check-1",
          eventType: "check_suite.completed",
          identifier: "getsentry/junior#1563",
          namespace: "github",
          occurredAtMs: baseMs,
          trustedSummary: "Check one failed",
        },
        receivedAtMs: baseMs,
        subscription,
        text: "Check one failed",
      });
      await appendAndEnqueueInboundMessage({
        conversationStore,
      ...
      // The lone event has not waited the 30s base debounce yet.
      await expect(
        processConversationQueueMessage(queue.takeMessage(), {
          conversationStore,
          queue,
      ...
      // A follow-up event during the wait nudges the wait later by one step
      // instead of restarting it, and does not schedule a second wake.
      nowSpy.mockReturnValue(baseMs + 200);
      const secondMessage = createEventInboundMessage({
        event: {
          eventKey: "check-2",
          eventType: "check_suite.completed",
          identifier: "getsentry/junior#1563",
          ...
          trustedSummary: "Check two failed",
        },
        receivedAtMs: baseMs + 200,
        subscription,
        text: "Check two failed",
      });
      await expect(
      ...
      // The original wake fires at baseMs + 30_000. Two pending events extend
      // the wait by one 5s step, so it defers again instead of running early.
      ...
      // Once the extended wait elapses, both events run together in one Turn.
      ...
      expect(transcript).toEqual(["Check one failed\n\nCheck two failed"]);
    } finally {
      nowSpy.mockRestore();
    }
  }, 10_000);
