import { defineConfig } from "tsup";

export default defineConfig({
  entry: {
    app: "src/app.ts",
    "cli/env": "src/cli/env.ts",
    "cli/main": "src/cli/main.ts",
    "cli/plugins": "src/cli/plugins.ts",
    "cli/init": "src/cli/init.ts",
    "cli/run": "src/cli/run.ts",
    "cli/chat": "src/cli/chat.ts",
    "cli/check": "src/cli/check.ts",
    "cli/upgrade": "src/cli/upgrade.ts",
    "cli/snapshot-warmup": "src/cli/snapshot-warmup.ts",
    "api/people/list": "src/api/people/list.ts",
    "api/people/profile": "src/api/people/profile.ts",
    "api/conversations/stats": "src/api/conversations/stats.ts",
    "api/conversations/list": "src/api/conversations/list.ts",
    "api/conversations/detail": "src/api/conversations/detail.ts",
    "api/conversations/subagent": "src/api/conversations/subagent.ts",
    instrumentation: "src/instrumentation.ts",
    nitro: "src/nitro.ts",
    reporting: "src/reporting.ts",
    vercel: "src/vercel.ts",
  },
  format: "esm",
  tsconfig: "tsconfig.build.json",
  dts: false,
  outDir: "dist",
  clean: true,
  splitting: true,
  external: [
    "#junior/config",
    "hono",
    "@sentry/node",
    // All runtime npm dependencies stay external
    "@ai-sdk/gateway",
    "@chat-adapter/slack",
    "@chat-adapter/state-memory",
    "@chat-adapter/state-redis",
    "@earendil-works/pi-agent-core",
    "@earendil-works/pi-ai",
    "@sinclair/typebox",
    "@slack/web-api",
    "@vercel/functions",
    "@vercel/queue",
    "@vercel/sandbox",
    "ai",
    "bash-tool",
    "chat",
    "commander",
    "just-bash",
    "node-html-markdown",
    "yaml",
    "zod",
  ],
});
