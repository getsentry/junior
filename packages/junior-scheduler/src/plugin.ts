  ...
  return defineJuniorPlugin({
    ...
    hooks: {
      ...
      systemPrompt(ctx) {
        ...
        return [
          {
            text: "Scheduled tasks make the task creator's connected credentials available by default when the requested work needs user-bound authorization. Do not ask for separate confirmation merely to use credentials needed for the requested work. On creation, use creator credential mode by default and system only when the creator explicitly requires it. For later changes, only the task creator may enable or re-enable creator credential use.",
          },
        ];
      },
      tools(ctx) {
