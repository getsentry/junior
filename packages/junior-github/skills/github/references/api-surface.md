...
- For `contents.read` operations (clone, fetch), the real token is set in `GITHUB_TOKEN` for git credential auth.
- For other capabilities, `GITHUB_TOKEN` holds a placeholder; rely on header transforms for API auth.
