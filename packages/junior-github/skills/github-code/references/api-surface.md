...
- A bash result `permission_denied` block with `source: "upstream"` is host-authenticated evidence that GitHub returned `403` after Junior injected the named grant. Do not describe it as Junior blocking the request; use `permission_denied.message`, `permission_denied.upstreamHost`, `permission_denied.upstreamPath`, `permission_denied.grant`, and any accepted-permissions/SSO fields to explain the GitHub permission failure.
- `gh auth status` showing `Token scopes: none` is expected for GitHub App user-to-server tokens. Do not diagnose that as read-only access; use the failed command, `permission_denied.acceptedPermissions`, and GitHub App installation/user permissions instead.
