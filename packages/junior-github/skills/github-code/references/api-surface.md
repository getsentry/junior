...
- To embed a local image in a GitHub issue, pull request, review, or comment, call `publishImage` first. That tool returns a durable public URL and markdown. The published image is public to anyone on the internet who has the URL. Do not use private Slack file links or conversation attachment URLs.
