...
| Fetch base branch for comparison   | `git -C DIRECTORY fetch origin BASE:refs/remotes/origin/BASE`                                                            |
| Deepen base and head history       | `git -C DIRECTORY fetch --deepen=N origin BASE:refs/remotes/origin/BASE BRANCH`                                          |
...
- Before rebasing, merge-base analysis, blame/history inspection, or a base comparison, check whether the repository is shallow. Fetch the base into `refs/remotes/origin/BASE`, deepen that ref incrementally until the needed ancestry is present, and compare against `origin/BASE`; use `--unshallow` only when bounded deepening is insufficient. Never force-push to work around missing ancestry.
