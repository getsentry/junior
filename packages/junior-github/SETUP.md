...
- Git commits use Junior as author and committer. Resolvable human run actors are credited once with `Co-Authored-By` trailers. Junior installs a sandbox-wide `core.hooksPath` for that attribution and forwards each client-side hook to the repository's matching `.git/hooks` entry when one exists.
