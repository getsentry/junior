...
const memoryRecallDecisionSchema = z
  .object({
    relevantIds: z
      ...
      .max(20)
      .describe(
        "Candidate ids whose memories would directly help with the current request, in relevance order.",
      ),
  })
  .strict();
...
export interface MemoryAgent {
  /** Select memories that directly help with the current user request. */
