...
9. Prefer one focused assertion path per behavior contract; add more cases only when they validate a distinct failure mode.
10. Workflow behavior integration tests should execute real runtime paths and only substitute deterministic fake agent/model output through explicit composition or named harness ports.
11. Observability must remain in the runtime path for behavior tests. Do not mock or disable Sentry capture, logging, span capture, or tracing modules to make ordinary tests easier.
12. Do not assert internal observability emission (`logInfo`, `logWarn`, spans, trace attributes) in behavior tests unless instrumentation output is itself the contract under test.
...
9. Integration tests must not use `vi.mock` or `vi.doMock`; inject deterministic behavior through local factories, role-named runtime adapters, `ReplyRequestContext.harness.streamFn`, or other named harness ports owned by the runtime contract.
