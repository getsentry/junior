...
1. User asks Junior to send files in thread:
   - When Junior needs to send, share, or attach sandbox-path files here, in this conversation, or in the current Slack thread, Junior may use `sendMessage` with `target: "thread"`.
