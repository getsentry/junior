...
- Raw sandbox GitHub issue creation is rejected before credential issuance. `POST /repos/:owner/:repo/issues` and GraphQL `createIssue` mutations must use the typed host/plugin operation `github.issue.create`, which owns idempotency and the runtime-generated conversation footer. Rejected raw issue creation returns HTTP 403, records no auth-required signal, and does not contact GitHub.
- Raw sandbox GitHub GraphQL `POST` bodies that exceed the inspection cap are rejected before credential issuance because Junior cannot prove they are not issue creation requests.
