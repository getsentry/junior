...
- Upstream `403` responses are permission denials for an issued lease, not missing authorization. They pass through raw, preserve the cached lease, and record a host-side `permission_denied` signal on failed bash results with provider, grant, upstream target, and provider permission headers such as GitHub's `X-Accepted-GitHub-Permissions` when present.
