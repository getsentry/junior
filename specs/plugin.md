...
7. Trusted runtime behavior is app-code registration, not manifest registration. Apps pass one `defineJuniorPlugins(...)` set to both `juniorNitro({ plugins })` and `createApp({ plugins })`.
