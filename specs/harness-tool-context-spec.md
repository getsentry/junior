...
4. Private bot-only artifact creation paths are disallowed for shared deliverables (for example Slack canvases for `/brief`).

## Slack-Specific Targeting Rules

1. Channel-scoped Slack tools use the active harness channel context.
2. Canvas creation uses shared conversation context only (`C*`/`G*` channel scope); DM/private-to-bot fallback is disallowed.
...
4. Disallowed fallback targets (for example bot-private canvases) are not invoked.
