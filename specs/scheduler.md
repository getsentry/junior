...
Model-facing scheduler create tools follow `policies/tool-design.md` and use an explicit `schedule_kind`: pass `one_off` for one-off tasks and `recurring` only when the user explicitly asks for a repeating task. For recurring create calls, also pass `daily`, `weekly`, `monthly`, or `yearly` as `recurrence`; for one-off create calls, omit `recurrence`, though `null` is accepted as equivalent to omitted. Model-facing update tools use the same small recurrence input for schedule edits, and pass `null` when updating an existing task to one-off. The scheduler derives stored calendar fields such as local start date, local time, weekday, month, and day-of-month from `nextRunAtMs` and timezone.
