...
- 2026-05-08: Added plugin-level `command-env` for non-secret sandbox CLI placeholders, default-backed deployment values, and explicitly exposed public host env values.
...
| `command-env`                           | `Record<string, string>` | Optional non-secret sandbox env vars injected when provider credentials or API headers are enabled. Requires `credentials` or `api-headers`. Values may reference `${NAME}` placeholders declared in `env-vars`; referenced env vars must declare defaults or `expose-to-command-env`. |
...
| `env-vars.<NAME>.default`               | `string`                 | Optional default value used by `mcp.url` or `command-env` when `process.env[NAME]` is unset or empty. Must be omitted for env vars referenced from `api-headers` and cannot be combined with `expose-to-command-env`.                                                                  |
| `env-vars.<NAME>.expose-to-command-env` | `boolean`                | Optional explicit permission for a non-secret host env var to be copied into sandbox `command-env`. Must only be used for values safe to expose to sandbox commands, such as Git author identity. Cannot be used by `api-headers`.                                                     |
...
default-backed `command-env` references. Command env references marked
`expose-to-command-env` are resolved from the host environment when sandbox
command env is built; if unset, that env entry is omitted. API header placeholders are
...
carry secrets, so their declarations must not include `default` or
`expose-to-command-env`. Missing env values fail when the provider's header
transforms are issued.
...
`env-vars`. Placeholder references must either declare `default` or set
`expose-to-command-env: true`. Exposed command env references are copied from
host env at command-env resolution time and are skipped when unset.
...
- If an `env-vars` entry sets `expose-to-command-env`, it must not set `default` and must not be referenced by `api-headers`.
...
- **No env var leakage.** Only non-secret placeholder/default command env values and explicitly exposed public host env values are injected into the sandbox. Secret-bearing provider values are delivered through host-managed header transforms.
